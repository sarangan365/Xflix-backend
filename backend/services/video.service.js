...
const getSearchedVideos = async (title, genres, sortBy, contentRating) => {
    try {
        ...
        // if there are no parameters are passed, then it return all videos 
        if (!title && !genres && !sortBy && !contentRating) {
            const videos = await Video.find({});
            return videos;
        }
        //To check whether it is title or genres or sortBy or content rating
        
        //if and only if the title is passed
        if (title && !genres && !sortBy && !contentRating) {
            const videos = await Video.find({ 
                "title": { 
        ...
        // if and only if sortedBy is passed
        if (sortBy) {
            const videos = await Video.find({});
            if (sortBy === "viewCount") {
        ...
        // if and only if contentRating is passed
